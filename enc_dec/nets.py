from keras.layers import Input, Conv2D, MaxPooling2D, UpSampling2D, Conv2D, Reshape, Conv2DTranspose, Flatten, Dense, \
    Conv1D
...
def get_fcn_vgg16_32s(shape, n_classes):
...
def get_segnet_vgg16(shape, n_classes):
    inputs = Input(shape=shape)
    x = BatchNormalization()(inputs)
    ...
    x = Conv2D(filters=n_classes, kernel_size=(11,11), strides=(20,20))(x)

    return Model(inputs=inputs, outputs=x)


def get_sampler(shape, n_classes):
    inputs = Input(shape=shape)
    x = BatchNormalization()(inputs)
    ...
    #x = Conv2D(filters=n_classes, kernel_size=(3,3))(x)

    return Model(inputs=inputs, outputs=x)


if __name__ == '__main__':
    model = get_sampler((500,500,3), 5)
    model.summary()