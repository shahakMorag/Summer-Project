...
def get_model():
    ...
    NN = max_pool_2d(NN, 2)
    NN = batch_normalization(NN)
    NN = conv_2d(NN, 16, 3, regularizer='L2', weight_decay=0.0001)
    ...
    NN = residual_block(NN, 5 - 1, 64)
    NN = batch_normalization(NN)
    NN = activation(NN, 'relu')
    NN = global_avg_pool(NN)
    NN = fully_connected(NN, 1024, activation='relu', weights_init='xavier', bias_init='xavier')
    NN = dropout(NN, 0.5)
    NN = fully_connected(NN, 1024, activation='relu', weights_init='xavier', bias_init='xavier')
    NN = dropout(NN, 0.5)
    ...
    NN = dropout(NN, 0.5)
    NN = fully_connected(NN, 5, activation='softmax')
    NN = regression(NN, optimizer='adam', learning_rate=0.008, loss='categorical_crossentropy', name='targets')
    return tflearn.DNN(NN, tensorboard_dir="../logs/", tensorboard_verbose=0)
